# -*- coding: utf-8 -*-
"""
D파트 "상황적용" 층 수집 — 찾기쉬운 생활법령정보(easylaw.go.kr) (작업단위 40)
==========================================================================
"원문 -> 해설 -> 상황적용" 3단 구조에서 비어있던 "상황적용" 레이어를, 법제처
찾기쉬운 생활법령정보의 '전세사기 피해자 지원'(csmSeq=1972) 책자형 콘텐츠로 채운다.
...
CSM_SEQ = 1972  # 전세사기 피해자 지원
UA = "Mozilla/5.0 (jeonse-easylaw-collector/1.0)"
REQUEST_DELAY_SEC = 1.0
OUT_NAME = "전세사기_피해자_지원.json"
...
def discover_leaves(landing_html: str) -> list[tuple[str, str, str]]:
    return sorted(set(_LEAF_RE.findall(landing_html)), key=lambda t: tuple(int(x) for x in t))
...
def run(out_dir: Path):
    ...
    print(f"랜딩에서 리프 페이지 발견: {landing}")
    leaves = discover_leaves(fetch(landing))
    ...
    record = {
        "출처": "법제처 찾기쉬운 생활법령정보(easylaw.go.kr) '전세사기 피해자 지원'",
        "이용조건": "공공누리 — 출처표기",
        "csmSeq": CSM_SEQ,
        "페이지수": len(pages),
        "페이지": pages,
    ...
    print(f"\n=== 완료: {len(pages)}페이지 -> {out_path} ===")


def main():
    p = argparse.ArgumentParser(description="easylaw 전세사기 생활법령 수집 (작업단위 40)")
