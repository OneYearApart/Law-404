...
from app.graph.parts.d_part.schemas import (  # noqa: E402
    SlotStatus,
    ...
    for scenario in scenarios:
        ...
        for index, turn in enumerate(scenario["turns"]):
            ...
            if ask in _SLOT_ORDER:
                if not unresolved:
                    errors.append(
                        f"{where}: expect_ask가 '{ask}'인데 미해결 슬롯이 없음"
                    )
                ...
                if unresolved:
                    errors.append(
                        f"{where}: expect_ask가 'relief'인데 미해결 슬롯이 남아있음 {unresolved}"
                    )
            elif ask is None:
                if not is_last:
                ...
                # fallback은 미해결 슬롯을 남긴 채 종결하는 게 정상이라 검사에서 뺀다
                    ...
                    errors.append(
                        f"{where}: 종결 턴인데 미해결 슬롯이 남아있음 {unresolved}"
                    )
        ...
        if outcome == "judgment":
            ...
            errors.append(
                f"{sid}: outcome '{outcome}'인데 expect_judgment가 null이 아님"
            )
...
async def _replay(scenario: dict) -> dict:
    ...
    for index, turn in enumerate(scenario["turns"]):
        ...
        if error is not None:
            turn_results.append(
                {"index": index + 1, "user": turn["user"], "error": error}
            )
            break

        slots = state.get("victim_slots") or VictimRequirementSlots()
        predicted = {
            name: getattr(slots, name).value if getattr(slots, name) else None
    ...
    judgment = (
        ...
        if turn_results and turn_results[-1].get("error") is None
        else None
    )
    return {
        "id": scenario["id"],
        ...
        "predicted_outcome": _observed_outcome(state)
        if turn_results[-1].get("error") is None
        else None,
        "expected_judgment": scenario["expect_judgment"],
...
def _score(results: list[dict]) -> dict:
    ...
    for scenario in results:
        for turn in scenario["turns"]:
            ...
            for name in _SLOT_ORDER:
                expected, predicted = (
                    turn["expected_slots"][name],
                    turn["predicted_slots"][name],
                )
                slot_total += 1
            ...
            auction_total += 1
            if (
                turn["expected_slots"]["auction_completed"]
                == turn["predicted_slots"]["auction_completed"]
            ):
                auction_hits += 1
            ask_total += 1
    ...
    judgment_hits = sum(
        s["expected_judgment"] == s["predicted_judgment"] for s in results
    )
    ...
    wrongful_exclusions = [
        s["id"]
        for s in results
        if s["predicted_outcome"] == "exclusion"
        and s["expected_outcome"] != "exclusion"
    ]
    latencies.sort()
    ...
    return {
        ...
        "auction_accuracy": round(auction_hits / auction_total, 4)
        if auction_total
        else None,
        ...
        "judgment_accuracy": round(judgment_hits / len(results), 4)
        if results
        else None,
...
async def run(scenarios: list[dict]) -> None:
    ...
    print(f"\n{'=' * 70}")
    print(
        f"D파트 피해자 판정 평가 — 시나리오 {metrics['n_scenarios']}개 / 턴 {metrics['n_turns']}개, 총 {wall:.1f}초"
    )
    print(f"{'=' * 70}")
    print(f"  최종 판정 정확도       {metrics['judgment_accuracy']:.1%}   ← 주지표")
    print(
        f"  종결 형태 정확도       {metrics['outcome_accuracy']:.1%}   (판정/제외/폴백)"
    )
    print(
        f"  슬롯 정확도            {metrics['slot_accuracy']:.1%}   (4슬롯 × {metrics['n_turns']}턴)"
    )
    ...
    for scenario in results:
        ...
        for turn in bad_turns:
            if turn.get("error") is not None:
                print(
                    f'    turn{turn["index"]} "{turn["user"]}" → 실패 {turn["error"]}'
                )
                continue
            ...
            if turn["expected_ask"] != turn["predicted_ask"]:
                print(
                    f"       다음질문: 기대 {turn['expected_ask']} / 실제 {turn['predicted_ask']}"
                )
            print(f"       ↳ {turn['note']}")
...
def main() -> int:
    ...
    parser.add_argument(
        "--run", action="store_true", help="실제 대화를 재생해 정확도 측정(과금)"
    )
