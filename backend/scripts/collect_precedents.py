...
def _search_law(oc: str, law: str, query_list: list[dict]) -> tuple[str, list[dict]]:
    ...
    results = []
    for q in query_list:
        page_results = rate_limited_call(
            search_all_pages_serial, oc, q["query"], q.get("JO")
        )
...
def run_track_c(oc: str, out_dir: Path):
    ...
    for law in TRACK_C_QUERIES:
        law_counts[law]["검색후보"] = sum(
            1 for c in candidates.values() if law in c["laws"]
        )
    ...
    for i, (pid, entry) in enumerate(candidates.items(), 1):
        ...
        if existing is not None:
            ...
            print(
                f"  [{i}/{len(candidates)}] 상세 조회: {pid} ({entry['meta']['사건명'][:40]})"
            )
            try:
        ...
        confirmed_laws = sorted(
            l for l in entry["laws"] if is_track_c_genuine(l, detail)
        )
        ...
        if existing is not None:
            ...
            prev_laws = set(
                filter(
                    None,
                    (row["근거법"].split(";") if row and row.get("근거법") else []),
                )
            )
        ...
        existing_by_pid[pid] = {
            "판례일련번호": pid,
            "사건명": entry["meta"].get("사건명"),
            "선고일자": entry["meta"].get("선고일자"),
            "tags": "",
            "grade": grade,
            "근거법": ";".join(confirmed_laws),
        }
    ...
    for law, counts in law_counts.items():
        print(
            f"  {law}: 검색후보 {counts['검색후보']}건 -> 근거법 인용 확정 {counts['확정']}건 (신규 저장 {counts['신규']}건)"
        )
...
def run_trust_law_recollect(oc: str, out_dir: Path):
    ...
    print("\n=== 신탁법 재수집 완료 ===")
    print(
        f"  신규 확정(A/B): {new_count}건 (기존 수집분과 중복 스킵: {skipped_existing}건)"
    )
    print("  검색어별 유효성:")
    for query, counts in query_stats.items():
        print(
            f'    "{query}": 검색후보 {counts["검색후보"]}건 -> 확정 {counts["확정"]}건'
        )
    print(f"  인덱스: {index_path}")
...
def is_ownership_change_lease_related(detail: dict) -> bool:
    ...
    return any(kw in haystack for kw in OWNERSHIP_LEASE_KEYWORDS) and any(
        kw in haystack for kw in OWNERSHIP_CHANGE_KEYWORDS
    )
...
def run_ownership_recollect(oc: str, out_dir: Path):
    ...
    print("\n=== 중-① 소유권변동 재수집 완료 ===")
    print(
        f"  신규 확정(A/B): {new_count}건 (기존 중복 스킵: {skipped_existing}건, 노이즈 필터 제외: {noise_filtered}건)"
    )
    for query, counts in query_stats.items():
        print(
            f'    "{query}": 검색후보 {counts["검색후보"]}건 -> 확정 {counts["확정"]}건'
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            req = Request(
                url,
                headers={
                    "User-Agent": "Mozilla/5.0 (jeonse-contract-data-collector/1.0)"
                },
            )
            ...
            if err_code or (
                root.tag not in ("PrecSearch", "PrecService", "LawService")
...
def search_precedents(
    oc: str, query: str, jo: str = None, page: int = 1
) -> tuple[list[dict], int]:
    """
...
def run_collection(oc: str, out_dir: Path, include_track_a: bool = True):
    ...
    if include_track_a:
        ...
        for q in TRACK_A_QUERIES:
            ...
            print(
                f"    -> {len(results)}건 (형태소 매칭 후보, 법령명 정확 인용 여부 확인 중)"
            )
            genuine = 0
            for r in results:
                pid = r["판례일련번호"]
                ...
                if is_track_a_genuine(detail):
                    ...
                    genuine += 1
            print(
                f"    -> 법령명 정확 인용 확인된 건: {genuine}건 (나머지는 형태소 노이즈로 제외)"
            )
    ...
    for i, (pid, entry) in enumerate(collected.items(), 1):
        ...
        if pid in pre_fetched_details:
            ...
            print(
                f"  [{i}/{len(collected)}] 상세 조회: {pid} ({entry['meta']['사건명'][:40]})"
            )
            try:
                detail = get_precedent_detail(oc, pid)
            except RuntimeError as e:
...
def main():
    ...
    parser.add_argument(
        "--oc", required=True, help="law.go.kr Open API 인증 ID (예: Whitecube1)"
    )
    parser.add_argument("--out", default="./data", help="출력 디렉토리 (기본: ./data)")
    parser.add_argument(
        "--skip-track-a", action="store_true", help="Track A 확인 쿼리 생략"
    )
    parser.add_argument(
        "--debug", action="store_true", help="원본 API 응답을 ./debug_raw/ 에 저장"
    )
    parser.add_argument(
        "--check",
        action="store_true",
