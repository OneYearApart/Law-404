...
def _source_type_totals() -> dict[str, int]:
    with get_engine().connect() as conn:
        return dict(
            conn.execute(
                text("SELECT source_type, count(*) FROM d_part_embeddings GROUP BY 1")
            ).all()
        )
...
def main() -> int:
    ...
    print(
        "    (트리거 계열은 일반 13개 항목 밖이라 정상 — 단위 34의 '별도 상수 분리' 대상)"
    )
    ...
    print("=" * 74)
    print(
        f"  {'카테고리':<26} {'원문(법령)':>10} {'해설(판례)':>10} {'해설(HUG)':>10} {'상황적용':>8}"
    )
    print("  " + "-" * 70)
    ...
    for name in SPECIAL_CASE_CATEGORIES:
        ...
        print(
            f"  {name:<26} {'벡터':>10} {case_law:>10} {hug:>10} {situation:>8}   (태그: {tag})"
        )

    print(
        "  [victim_interview] 전세사기피해자법 제3조 직접 매핑 + 링크 조인 (topic_tags 미사용)"
    )
    ...
    if zero_coverage:
        ...
        print(
            "  -> 이 항목들은 판례/HUG 검색이 항상 0건이고 조문만으로 답변이 생성된다."
        )
