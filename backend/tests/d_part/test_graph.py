...
@pytest.mark.asyncio
async def test_full_graph_continues_pending_relief_question_to_judgment_response(
    monkeypatch,
):
    ...
    async def _unreachable_call_supervisor(user_input: str) -> dict:
        raise AssertionError(
            "진행 중인 흐름이 있을 땐 supervisor가 LLM을 호출하면 안 된다"
        )

    async def _fake_search_by_requirement(slots, situation_query=None):
        return {
            "statute": [Chunk(id=1, source_type="법령원문", content="제3조")],
            "case_law": [],
            "cases": [],
            "guides": [],
        }
...
@pytest.mark.asyncio
async def test_full_graph_matches_topic_from_a_different_stage_than_confirmed(
    monkeypatch,
):
...
@pytest.mark.asyncio
async def test_full_graph_routes_unmatched_question_to_open_qa_instead_of_fallthrough(
    monkeypatch,
):
...
async def test_full_graph_special_case_still_gets_its_guidance(monkeypatch):
    ...
    monkeypatch.setattr(
        special_cases.llm_d_part, "generate_response", _fake_generate_response
    )
...
@pytest.mark.asyncio
async def test_full_graph_attaches_support_guidance_on_general_scenario_when_recognized(
    monkeypatch,
):
...
@pytest.mark.asyncio
async def test_full_graph_attaches_nothing_on_general_scenario_when_not_recognized(
    monkeypatch,
):
        ...
        pytest.param(
            {
                ...
                "victim_slots": VictimRequirementSlots(
                    moved_in_and_fixed_date=SlotStatus.UNCLEAR
                ),
            },
            id="fallback",
        ),
...
async def test_full_graph_reclassifies_followup_after_victim_flow_closed(
    monkeypatch, closed_session
):
    ...
    async def _unreachable_search_by_requirement(slots):
        raise AssertionError(
            "종결된 인터뷰의 후속 턴에서 판정 응답을 재조립하면 안 된다"
        )
    ...
    monkeypatch.setattr(
        response_assembly._retriever,
        "search_by_requirement",
        _unreachable_search_by_requirement,
    )
...
async def test_first_turn_answers_directly_without_stage_gate(monkeypatch):
    ...
    async def _fake_search_by_topic(topic_key, query_text):
        return {
            "statute": [
                Chunk(id=1, source_type="법령원문", content="등기부 관련 조문")
            ],
            "case_law": [],
            "cases": [],
            "guides": [],
