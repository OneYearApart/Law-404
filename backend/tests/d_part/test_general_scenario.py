...
async def test_noop_when_final_answer_already_set():
    ...
    state = {
        "user_input": "네",
        "situation": SituationState(topic="전-①등기부등본_위험신호"),
...
@pytest.mark.asyncio
async def test_context_carries_both_topic_and_user_question(monkeypatch):
    """항목만 넘기면 같은 항목의 모든 발화가 같은 일반론을 받는다 — 사용자가 그 항목 안에서
    구체적으로 뭘 물었는지도 함께 넘긴다."""
    seen = {}
    ...
    state = {
        "user_input": "근저당이 3개나 잡혀 있는데 계약해도 되나요",
        "situation": SituationState(topic="전-①등기부등본_위험신호"),
    }
    result = await general_scenario.handle_general_scenario(state)
