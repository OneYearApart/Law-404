...
async def test_row_counts(ingested):
    ...
    assert (
        counts["판례"] == 467
    )  # 중-①소유권변동 임대인지위 승계 판례 재수집 +36 (작업단위 42)
    assert counts["법령원문"] == 932  # 시행령 3종(38) + 민사절차 3법(39) 포함
    assert counts.get("HUG사례집", 0) + counts.get("HUG규정", 0) == 176
    assert (
        counts["정부자료"] == 21
    )  # 깡통전세 유형/예방 8 + 실태조사 섹션 13 (작업단위 37)
    assert (
        counts["생활법령"] == 52
    )  # 전세사기 지원 20p(40) + 주택임대차 booklet 31p→분할 후 32행(작업단위 50)
...
async def test_easylaw_docs_loaded(ingested):
    ...
    assert (
        by_booklet["629"] == 31
    )  # 주택임대차 31p — 1p가 8000토큰 초과로 2행 분할돼 32행/31문서
    assert len(rows) == 51
...
async def test_links_loaded(ingested):
    with get_engine().connect() as conn:
        by_type = dict(
            conn.execute(
                text("SELECT linked_type, count(*) FROM d_reference_links GROUP BY 1")
            ).all()
        )
...
async def test_embedding_columns_null_before_embed(ingested):
    with get_engine().connect() as conn:
        remaining = conn.execute(
            text("SELECT count(*) FROM d_part_embeddings WHERE embedding IS NOT NULL")
        ).scalar_one()
    assert remaining == 0
