...
async def user_id():
    db = SessionLocal()
    user = User(
        username="test_live_smoke_user",
        password_hash="x",
        nickname="test_live_smoke_nick",
    )
    db.add(user)
    db.commit()
    db.refresh(user)
...
@pytest.mark.parametrize(
    "user_input", ["네", "넵", "옙", "ㅇㅇ", "맞아요", "그렇죠", "어 맞아", "y"]
)
...
@pytest.mark.asyncio
@pytest.mark.parametrize(
    "user_input", ["아니요", "아니에요", "틀렸어요", "아닌데요", "n"]
)
...
@pytest.mark.parametrize(
    "user_input",
    ["맞긴 한데 좀 애매해요", "아마도요", "잘 모르겠어요", "그건 왜 물어보세요?"],
)
...
async def test_generate_response_live():
    chunks = [
        c
        async for c in llm_d_part.generate_response(
            "판단 결과: 높음\n요건 슬롯: 전부 충족"
        )
    ]
    assert len(chunks) > 0
