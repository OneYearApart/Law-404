...
async def user_id():
    db = SessionLocal()
    user = User(
        username="test_summarizer_user",
        password_hash="x",
        nickname="test_summarizer_nick",
    )
    db.add(user)
    db.commit()
    db.refresh(user)
...
async def test_maybe_summarize_skips_when_below_threshold(user_id, monkeypatch):
    ...
    await summarizer.maybe_summarize_conversation(
        conversation.id, user_id, turn_threshold=4
    )
...
@pytest.mark.asyncio
async def test_maybe_summarize_triggers_and_saves_title_at_threshold(
    user_id, monkeypatch
):
    async def fake_summarize(messages):
    ...
    await summarizer.maybe_summarize_conversation(
        conversation.id, user_id, turn_threshold=2
    )

    db = SessionLocal()
    title = (
        db.query(Conversation).filter(Conversation.id == conversation.id).first().title
    )
    db.close()
    assert title == "요약된 제목"
...
async def test_maybe_summarize_leaves_title_unchanged_on_failure(user_id, monkeypatch):
    ...
    await summarizer.maybe_summarize_conversation(
        conversation.id, user_id, turn_threshold=1
    )
