...
DEFAULT_FIRST_QUESTION = (
    "계약 종료일이 2027년 3월 1일인데 갱신요구권 일정도 캘린더에 등록할 수 있나요?"
)
...
async def run_test(first_question: str, approval_message: str, top_k: int) -> None:
    ...
    print_json_section(
        "1턴 Calendar Event Candidates", first_state.get("calendar_events", [])
    )
    ...
    if not pending_action:
        ...
        print("[테스트 중단]")
        print(
            "1턴에서 pending_action이 생성되지 않았습니다. 계약 종료일이 인식되는 질문인지 확인하세요."
        )
        return
    ...
    print_json_section(
        "2턴 Calendar Registration", second_state.get("calendar_registration")
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="B파트 캘린더 pending_action 2턴 테스트"
    )
