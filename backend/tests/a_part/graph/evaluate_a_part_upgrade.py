...
PARAPHRASE_QUESTIONS = [
    ...
    ("q04_broker_account_payment", "부동산 사장님 통장으로 계약금을 먼저 넣으라고 해요."),
    ("q05_account_change_before_contract", "아까 받은 계좌 말고 다른 통장으로 보내 달라고 갑자기 바꿨어요."),
    ("q06_broker_explanation_mismatch", "확인설명서에는 없던 내용이 계약서에 적혀 있습니다."),
    ("q07_mortgage", "은행 빚이 잡힌 집인데 채권최고액이 커 보여요."),
    ("q08_multiunit_priority", "원룸 건물인데 기존 세입자 보증금 합계를 집주인이 안 알려줘요."),
    ("q09_registry_restriction_warning", "갑구에 압류 표시가 있는데 그냥 계약해도 되나요?"),
    ...
    ("q14_special_clause_deposit_return", "문제 생기면 계약금을 반환한다는 조항을 넣고 싶습니다."),
    ("q15_after_contract_procedure", "도장 찍고 계약한 다음에 바로 해야 하는 일이 뭐예요?"),
    ...
    ("q19_deposit_transfer_mismatch", "계약서에는 1억인데 송금한 금액은 9천만 원으로 보여요."),
...
def run_routing_comparison() -> dict[str, Any]:
    cases = [
        *[(issue, "representative", question) for issue, question in REPRESENTATIVE_QUESTIONS],
        *[(issue, "paraphrase", question) for issue, question in PARAPHRASE_QUESTIONS],
        *[("OUT_OF_SCOPE", "out_of_scope", question) for question in OUT_OF_SCOPE_QUESTIONS],
    ]

    all_rows: list[dict[str, Any]] = []
    summaries: dict[str, Any] = {}
    for mode_name, use_langchain in [("legacy_rule", False), ("langchain_upgrade", True)]:
        ...
        summaries[mode_name] = {
            ...
            "representative_accuracy_percent": rate(
                [row for row in mode_rows if row["group"] == "representative"], "correct"
            ),
            ...
            "langchain_engine_percent": round(
                sum(row["engine"] == "langchain" for row in mode_rows) / len(mode_rows) * 100,
                2,
            ),
            "fallback_percent": round(
                sum("fallback" in str(row["engine"]) for row in mode_rows) / len(mode_rows) * 100,
                2,
            ),
...
def _run_live_mode(mode: str, *, use_langgraph: bool) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    from app.consultation.a_part.chatbot_service import APartChatbotService, ChatbotTurnRequest
    ...
    for expected_issue, question in REPRESENTATIVE_QUESTIONS:
        ...
        try:
            ...
            result = service.handle(request) if use_langgraph else service._handle_without_langgraph(request)
            ...
            immediate_actions = list(getattr(answer, "immediate_actions", []) or []) if answer else []
            hold_actions = list(getattr(answer, "hold_actions", []) or []) if answer else []
            reasons = list(getattr(answer, "reasons", []) or []) if answer else []
            core_judgment = str(getattr(answer, "core_judgment", "") or "") if answer else ""
            ...
            row = {
                ...
                "framework": result.orchestration.get("framework") if result.orchestration else "legacy",
                "graph_ok": result.orchestration.get("framework") == "langgraph" if result.orchestration else False,
                "route_engine": result.orchestration.get("route_engine") if result.orchestration else "legacy_rule",
                "planner_engine": result.orchestration.get("query_planner_engine") if result.orchestration else "legacy_query",
                "document_mode": result.orchestration.get("document_mode") if result.orchestration else "general",
                "answer_ready": result.answer_ready,
                "generation_status": generation_status,
                "generation_completed": generation_status == "completed",
                "evidence_status": evidence_status,
                "search_query": query_used,
                "query_rewritten": " ".join(query_used.split()) != " ".join(question.split()),
                ...
                "structure_complete": bool(core_judgment.strip() and immediate_actions and reasons),
                "follow_up_count": follow_up_count,
                "one_follow_up_ok": follow_up_count <= 1,
                "expect_hold": expect_hold,
                "safety_hold_ok": (not expect_hold) or bool(hold_actions),
                "risk_level": str(getattr(answer, "risk_level", "") or "") if answer else "",
                "question": question,
                "error": "",
            }
        except Exception as exc:  # noqa: BLE001
    ...
    summary = {
        ...
        "langchain_planner_success_percent": round(
            sum(row["planner_engine"] == "langchain" for row in rows) / len(rows) * 100, 2
        ),
...
def run_live_e2e(*, compare_legacy: bool = False) -> dict[str, Any]:
    ...
    if compare_legacy:
        ...
        summaries["improvement"] = {
            "routing_accuracy_percentage_point": round(new["routing_accuracy_percent"] - old["routing_accuracy_percent"], 2),
            "generation_completed_percentage_point": round(new["generation_completed_percent"] - old["generation_completed_percent"], 2),
            "evidence_found_percentage_point": round(new["evidence_found_percent"] - old["evidence_found_percent"], 2),
            "structured_answer_complete_percentage_point": round(new["structured_answer_complete_percent"] - old["structured_answer_complete_percent"], 2),
            "high_risk_hold_action_percentage_point": round(new["high_risk_hold_action_percent"] - old["high_risk_hold_action_percent"], 2),
            "average_selected_evidence_difference": round(new["average_selected_evidence_count"] - old["average_selected_evidence_count"], 2),
            "average_latency_difference_ms": round(new["average_latency_ms"] - old["average_latency_ms"], 2),
...
def run_multiturn() -> dict[str, Any]:
        ...
        rows.append(
            {
                ...
                "active_issue_kept": result.consultation.primary_issue_id == "q01_owner_proxy",
    ...
    summary = {
        ...
        "duplicate_question_percent": round(duplicates / len(question_keys) * 100, 2) if question_keys else 0.0,
        "final_missing_fact_count": rows[-1]["missing_fact_count"],
        "final_follow_up_count": rows[-1]["follow_up_count"],
        "average_latency_ms": round(statistics.mean(row["latency_ms"] for row in rows), 2),
    }
...
def run_pdf(lease_pdf: str | None, registry_pdf: str | None) -> dict[str, Any]:
    ...
    if not lease_pdf and not registry_pdf:
        return {"skipped": True, "reason": "--lease-pdf 또는 --registry-pdf를 지정하지 않았습니다."}
    ...
    if lease_pdf and registry_pdf:
        question = "첨부한 계약서와 등기부등본을 비교해서 다른 내용과 위험 요소를 알려주세요."
    ...
    summary = {
        ...
        "document_mode_correct": result.orchestration.get("document_mode") == expected_mode,
        "framework": result.orchestration.get("framework"),
        "analysis_created": analysis is not None,
        "lease_analysis_count": len(getattr(analysis, "lease_analyses", []) or []) if analysis else 0,
        "registry_analysis_count": len(getattr(analysis, "registry_analyses", []) or []) if analysis else 0,
        "comparison_created": bool(getattr(analysis, "comparison", None)) if analysis else False,
...
def main() -> None:
    ...
    if not results:
        parser.error("--routing, --live, --live-compare, --multiturn, --all 또는 PDF 경로를 지정하세요.")
