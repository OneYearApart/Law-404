...
| `col-` | 24 | **구어 발화** — 13개 항목을 법률 용어 없이 표현. 어휘 간극 누수 측정용 |
| `hld-` | 12 | **홀드아웃** — 프롬프트 힌트에도 `col-`에도 없는 제3의 표현 |

`--run`은 접두어별 경로 정확도를 따로 낸다. 전체 평균에 섞으면 교과서적 발화(`gen-`)가 구어
실패를 덮어버려서, 실제 사용자가 겪는 누수가 보이지 않는다.

⚠️ **`col-`은 오염된 지표다.** 이 케이스들의 표현을 진단에 쓴 뒤 그대로 `supervisor.md`의 항목
힌트로 넣었기 때문에, `col-` 점수는 사실상 in-sample이라 부풀려져 있다. **개선폭을 인용할 때는
`hld-`를 쓸 것.** 앞으로 프롬프트에 예시를 추가할 때마다 같은 오염이 생기므로, 그때마다 새로운
홀드아웃을 만들어야 한다.
