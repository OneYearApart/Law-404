...
def upgrade() -> None:
    ...
    op.add_column(
        "legal_documents_c", sa.Column("court_level", sa.Integer(), nullable=True)
    )
    op.add_column(
        "legal_documents_c", sa.Column("case_year", sa.Integer(), nullable=True)
    )
    op.add_column(
        "legal_documents_c", sa.Column("ruling_type", sa.String(50), nullable=True)
    )
    op.add_column(
        "legal_documents_c", sa.Column("appeal_outcome", sa.String(50), nullable=True)
    )
    op.add_column(
        "legal_documents_c", sa.Column("related_procedures", sa.Text(), nullable=True)
    )

    op.create_index(
        "idx_legal_documents_c_court_level", "legal_documents_c", ["court_level"]
    )
    op.create_index(
        "idx_legal_documents_c_case_year",
        "legal_documents_c",
        ["case_year"],
        postgresql_ops={"case_year": "DESC"},
    )
    op.create_index(
        "idx_legal_documents_c_ruling_type", "legal_documents_c", ["ruling_type"]
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "lawyer_fee_rate_min", sa.Integer(), nullable=False, server_default="1"
        ),
        sa.Column(
            "lawyer_fee_rate_max", sa.Integer(), nullable=False, server_default="3"
        ),
