...
def detect_primary_issue_id(query: str) -> str:
    ...
    if _contains_any(
        text, ["공동명의", "공동소유자", "소유자가 두 명", "소유자 한 명만"]
    ):
        return "q02_co_owner"
    ...
    if _contains_any(
        text, ["부동산 계좌", "중개사 계좌", "제3자 계좌", "계좌로 계약금"]
    ):
    ...
    if _contains_any(
        text, ["집주인 아들", "대신 계약", "위임장 없이", "대리 권한", "대리인"]
    ):
    ...
    if _contains_any(
        text, ["집주인이 바뀌", "소유자가 바뀌", "소유자 변경", "새 소유자"]
    ):
    ...
    if _contains_any(
        text,
        ["계약서 쓴 직후", "계약서 작성 직후", "계약서를 쓴 뒤", "바로 해야 할 절차"],
    ):
    ...
    if (
        ...
        and "보증금" in text
        and _contains_any(
            text, ["이체 내역", "이체내역", "송금 내역", "금액이 다", "차액"]
        )
    ):
        return "q19_deposit_transfer_mismatch"
    if _contains_any(
        text, ["전세보증금반환보증", "보증보험 가입 전", "반환보증 가입", "보증보험"]
    ):
    ...
    document_markers = ("계약서", "등기부", "등기사항증명서", "pdf", "첨부", "업로드")
    review_markers = (
        "확인",
        "검토",
        "분석",
