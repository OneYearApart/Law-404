...
from app.graph.parts.c_part.graph import graph as c_graph

router = APIRouter(prefix="/chat/c", tags=["c_part"])


@router.post("/")
async def chat_c(request: dict, user=Depends(get_current_user_optional)):
    async def event_generator():
        yield f"data: {StreamEvent(type=EventType.LOADING).model_dump_json()}\n\n"

        # 판단 단계(전/중/후, 위험신호 감지 등)는 내부적으로 동기 실행
        final_state = await c_graph.ainvoke(request)
        ...
        if user:
            await save_message(user.id, "c", "assistant", final_state.get("final_answer", ""))
