...
class CreateConversationResponse(BaseModel):
    ...
    conversation_id: int = Field(
        ..., description="생성된 대화 ID. 이후 요청에 사용하세요."
    )
...
class AskRequest(BaseModel):
    ...
    conversation_id: int = Field(
        ..., description="대화 ID (먼저 /conversations로 생성)"
    )
...
class AskResponse(BaseModel):
    ...
    response_type: str = Field(
        ...,
        description="definition | off_topic | consultation | guide | document_intent ",
    )
...
def _convert_search_results(chunks: list) -> dict:
    ...
    for chunk in chunks:
        if chunk.source_type == "statute":
            ...
            article_num = (
                f"{article_num}조의{chunk.statute_branch}"
                if chunk.statute_branch
                else f"{article_num}조"
            )
            statutes.append(
                {
...
async def ask(
    ...
    try:
        ...
        logger.info(
            f"[C파트] 상담 완료 (신뢰도 {answer.get('confidence_score', 0):.2f}, "
            f"{elapsed:.1f}초)"
        )
    ...
    try:
        ...
        async for update in graph.astream(
            ...
            for node_name, node_out in update.items():
                ...
                # ── FAQ 노드 ──
                if (
                    "follow_up_questions" in node_out
                    and node_out["follow_up_questions"] is not None
                ):
        ...
        saved_text = _assemble_answer_text(
            {  # noqa: F821
                **collected_sections,
            }
        )
        if saved_text:
            await conv_repo.save_message(
                user_id, PART, "assistant", saved_text, conv_id
            )  # noqa: F821
        ...
        if not meta_emitted:
            yield sse_event(
                "meta",
                {
                    "confidence_score": _confidence_from_sections(
                        collected_sections, search_results
                    ),
...
async def create_document(
    ...
    try:
        ...
        # 【4】대화 저장
        await conv_repo.save_message(
            user.id, PART, "user", request.user_message, conv_id
        )
        assistant_msg = result.get("next_question") or result.get("document") or ""
        if assistant_msg:
            await conv_repo.save_message(
                user.id, PART, "assistant", assistant_msg, conv_id
            )

        logger.info(
            f"[C파트] 문서 {result['status']} (진행률 {result['progress']:.0%})"
        )
...
async def create_document_from_image(
    ...
    try:
        ...
        if not agent.ocr.is_available():
            raise HTTPException(
                503, "OCR 기능이 설정되지 않았습니다. 텍스트로 직접 입력해 주세요."
            )
        ...
        logger.info(
            f"[C파트] OCR 문서 {result['status']} (이미지에서 {len(found)}개 추출)"
        )
