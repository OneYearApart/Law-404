...
)  # retriever 내부 로그는 잠시 끔 (결과만 깔끔하게 보려고)
...
def run_validation():
    ...
    for tc in TEST_CASES:
        ...
        print(
            f"  조문 기대값: 제{tc.expected_statute}조"
            if tc.expected_statute
            else "  조문 기대값: (없음)"
        )
        print(
            f"  → 검색 결과: {[s.statute_number + (f'의{s.statute_branch}' if s.statute_branch else '') for s in result['statutes']]}"
        )
    ...
    if statute_hits < total or case_hits < total:
        ...
        print(
            "   3. top_k를 늘려서 몇 위에 있는지 확인 (완전히 없는 것과 순위가 낮은 건 다른 문제)"
        )
