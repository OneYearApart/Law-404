...
class RetrievalResult(BaseModel):
    ...
    precedents: list[dict]  # [{"case_number": "2023다202228", "content": "...", ...}, ...]
...
class AnswerGeneratorInput(BaseModel):
    ...
    chat_history: Optional[list[Message]] = Field(None, description="대화 히스토리 (나중에 추가)")
...
class AnswerSection(BaseModel):
    ...
    citations: list[str] = Field(default_factory=list, description="근거 출처 (조문/판례 사건번호)")
...
class AnswerOutput(BaseModel):
    ...
    legal_basis: AnswerSection = Field(..., description="관련 법 조문 - 조문 원문 + 해석")
    precedents: AnswerSection = Field(..., description="관련 판례 - 판례 분석 + 유사점")
    action_steps: AnswerSection = Field(..., description="구체적 행동 절차 - 단계별 가이드")
    expected_cost: AnswerSection = Field(..., description="예상 비용 - 경로별 비용 계산")
    anticipated_disputes: AnswerSection = Field(..., description="임대인 반박 & 대응 - 예상 분쟁 대응")
    ...
    confidence_score: float = Field(..., description="신뢰도 점수 (0.0~1.0, 근거 데이터 충분도)")
