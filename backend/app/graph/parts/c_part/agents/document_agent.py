...
파일 위치: app/graph/parts/c_part/agents/document_agent.py

【이 에이전트가 기존 AnswerGenerator와 다른 점】
...
# ════════════════════════════════════════════════════════════════════════════════
# ⚠️ 필수/선택을 나눈 기준:
#    필수 = 없으면 법적 효력이 의심스럽거나 문서가 성립하지 않는 것
#    선택 = 있으면 좋지만 없어도 문서는 만들어지는 것
#
#    되묻기는 '필수'만 합니다.
...
# ════════════════════════════════════════════════════════════════════════════════
# ⚠️ 왜 GPT로 파싱하나?
#    사용자가 "집주인은 김철수고 보증금 5천만원이에요"처럼
#    여러 정보를 한 문장에 섞어서 말합니다.
#    정규식으로는 못 잡습니다.
#
# ⚠️ JSON으로만 답하게 강제하는 이유:
#    파싱해야 하므로 설명이 섞이면 안 됩니다.
...
class DocumentAgent:
    ...
    3가지 능력:
      1. extract_info()      사용자 메시지에서 정보 추출
      2. ask_next()          부족한 정보를 되묻기
      3. generate_document() 문서 생성

    그리고 이 셋을 조합하는:
      4. process()           "다음에 뭘 할지" 스스로 판단  ← 에이전트의 핵심
    ...
    # → {"status": "need_more_info", "next_question": "임차인 성함이...", ...}

    # 사용자가 답하면
    result = await agent.process(
        user_message="저는 홍길동이고 집주인은 김철수예요",
        collected=result["collected"],   # ← 프론트가 들고 있던 것
    )
    # → 정보가 2개 추출됨. 또 물어봄.
    ...
    # 정보가 다 모이면
    # → {"status": "complete", "document": "내 용 증 명\n\n..."}
    ```
    """
    ...
    def __init__(self, llm: BaseLanguageModel, ocr: Optional[ClovaOCR] = None):
            ...
            Args:
                llm: ChatOpenAI 인스턴스
                ocr: 클로바 OCR 클라이언트. 없으면 새로 만듦.
                    (테스트에서 Mock을 주입할 수 있게 파라미터로 뺌)
            """
            self.llm = llm
    ...
    async def extract_info(
        ...
        """
        【추출】사용자 메시지에서 정보를 뽑아 collected에 병합
        ...
        ⚠️ 기존 값을 덮어쓸 수 있습니다.
           사용자가 "아 이름 잘못 말했어요, 김영희예요"라고 하면
           landlord_name이 갱신되어야 합니다.
        """
