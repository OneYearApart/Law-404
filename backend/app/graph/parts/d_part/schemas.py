...
SPECIAL_CASE_CATEGORIES: tuple[str, ...] = (
    "임대인 사망/파산",
    "신탁사기",
    "다가구주택",
    "공인중개사 허위고지",
...
SITUATION_TOPIC_TO_SPECIAL_CASE: dict[str, str] = {
    topic: case
    for case, topic in SPECIAL_CASE_TOPIC_TAGS.items()
    if topic in GENERAL_TOPIC_LABELS
}
...
class VictimRequirementSlots(BaseModel):
    ...
    multiple_victims_reason: Optional[str] = (
        None  # ③의 구체적 근거(파산회생/경공매개시/압류/집행권원 등) 텍스트 기록
    )
    # 아래 두 필드는 ①~④ 요건과 별개의 통제 플래그 (참고: D파트_구현_종합문서.md §9.1)
    auction_completed: Optional[bool] = (
        None  # 경공매완료여부 — true면 ①③ 자유서술만으로 자동충족 처리
    )
    has_relief_measure: Optional[bool] = (
        None  # 구제수단보유여부 — true면 판단 결과를 "제외"로 덮어씀. 자유서술 추론 금지, 명시 확인질문 필수
    )
...
class SituationState(BaseModel):
    ...
    recognized: Optional[bool] = (
        None  # 피해자로 인정받은 상태인지 — topic과 독립적으로 판별
    )
...
class DPartGraphState(TypedDict, total=False):
    ...
    victim_pending_slot: Optional[
        str
    ]  # 직전 턴에 질문을 던진 슬롯 이름 — 이번 턴 발화가
    # "아니요" 같은 맥락 의존 답변일 때 어느 슬롯에 대한
    # 답인지 추출기에 알려주는 용도. carryover 필수
    awaiting_relief_confirmation: (
        bool  # 구제수단보유여부 명시적 질문에 대한 응답을 기다리는 중인지
    )
    ...
    retrieved_chunks: list[
        dict[str, Any]
    ]  # RAG 검색 결과 — 이번 턴 전용, carryover 아님
    # TODO: app/rag 쪽에 전용 Chunk 타입이 생기면 list[Chunk]로 교체 (2026-07-11 기준 미존재)
    final_answer: Optional[
        str
    ]  # 이번 턴 최종 답변 — messages 테이블에 별도 저장되므로 state에 중복 보관 안 함
    disclaimer_required: (
        bool  # 이번 턴 final_answer가 법률 정보 응답이라 면책 문구가 필요한지
    )
    # (확인질문/fallback엔 False). LLM 스트림 경로는 finalize가 항상 첨부. 이번 턴 전용
    response_stream: Optional[
        AsyncIterator[str]
    ]  # 스트리밍 청크 제너레이터 — JSON 직렬화 불가, 절대 영속화 금지
