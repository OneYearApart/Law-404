...
_REASK_MESSAGE = "죄송합니다, 다시 한번 현재 상황을 말씀해 주시겠어요?"
...
async def route_stage(state: DPartGraphState) -> DPartGraphState:
    ...
    상태기계:
      ...
      stage 있음 + stage_confirmed=False → 이번 턴 입력을 확인 응답으로 해석(yes/no/unclear)
    """
    ...
    if state.get("stage") is None:
        ...
        state["stage_confirm_attempts"] = 0
        state["final_answer"] = _confirmation_question(stage)
        state["active_query"] = user_input
        return state

    question = _confirmation_question(state["stage"])
    answer = await parse_confirmation(question, user_input)
    ...
    if answer is True:
        ...
        state["stage_confirmed"] = False
        state["stage_confirm_attempts"] = 0
        state["final_answer"] = _REASK_MESSAGE
        state["active_query"] = None
    else:
        attempts = state.get("stage_confirm_attempts", 0) + 1
        state["stage_confirm_attempts"] = attempts
        if attempts >= _MAX_CONFIRM_ATTEMPTS:
            # 확인을 못 받아낸 채 무한 재질문에 갇히느니 판별값을 그대로 확정하고 통과시킨다.
            # stage는 현재 stage_router 밖에서 아무도 소비하지 않으므로 오확정의 실질 피해가
            # 없다 — stage 자체의 존폐는 단위 29에서 결정한다.
            state["stage_confirmed"] = True
            state["final_answer"] = None
        else:
