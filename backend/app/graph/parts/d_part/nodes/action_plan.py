...
결정론적 액션플랜 텍스트를 만들어 state["appendix_text"]에 담는다. finalize가 스트림 말미에
...
async def attach_action_plan(state: DPartGraphState) -> DPartGraphState:
    ...
    제외/fallback)는 needs_response_assembly가 falsy이므로 no-op으로 통과 → appendix_text 미설정
    ...
    if judgment is None:
        return state
    state["appendix_text"] = build_action_plan(judgment, state["victim_slots"])
    return state
