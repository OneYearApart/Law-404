...
5. open_qa — 위 어디에도 해당하지 않는 질문에 대한 제약 없는 RAG 검색+응답 생성
6. response_assembly — victim_check가 방금 판단을 확정한 턴에만 원문→해설→상황적용 조립
7. action_plan — 그 판정 확정 턴에만 개요 수준 지원절차 안내를 조립(finalize가 면책 앞에 첨부).
