...
14. 이미 확인된 사실은 사용자의 실제 답변이므로 일반적인 상황 설명보다 우선한다.
15. "아니요"와 "확인하지 못함"을 임의로 "확인됨" 또는 "해당 없음"으로 바꾸지 않는다.
16. 핵심 판단에는 내부 분류명, 대괄호 이슈명, 슬롯명, "남은 핵심 항목" 같은 시스템 문구를 쓰지 않는다.
17. 확인된 사실끼리 모순이 있으면 안전을 단정하지 말고 어떤 사실을 다시 대조해야 하는지 사용자 문장으로 설명한다.
18. 위임 자료가 없다는 사실만으로 무권대리, 계약 무효, 무효 가능성이 높다고 단정하지 않는다. 실제 대리권과 권한 범위를 확인하지 못한 상태라고 표현한다.
19. 무권대리가 문제 되는 경우에도 "대리인이 계약의 효력을 가진다"고 표현하지 않는다. 실제 대리권이 없고 본인의 추인도 없는 경우 본인에 대한 효력 귀속이 문제 된다고 설명한다.
20. 이미 확인한 소유자·권한을 처음부터 다시 확인하라는 일반 문장보다, 아직 확인하지 못한 권한·자료·계좌를 해결하는 구체적인 행동을 작성한다.
...
def _apply_server_side_safety_rules(
    ...
    if policy_id:
        ...
        if policy_payload:
            # 첫 질문에는 검증된 고정 정책 답변을 유지한다.
            # 후속 질문으로 실제 사실이 모인 뒤에는 고정 템플릿이 사용자 답변을
            # 덮어쓰지 않도록 근거 기반 구조화 답변을 그대로 사용한다.
            if not consultation_context.known_facts:
