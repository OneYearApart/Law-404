...
사용자의 질문을 세 가지 중 하나로 분류하세요.

【분류 기준】

1. CONSULTATION (상담)
   보증금 반환·경매·배당에 대해 "묻거나 설명을 원하는" 경우
   예:
   - "보증금을 못 받았는데 어떻게 하나요?"
   - "임차권등기명령이 뭔가요?"
   - "경매 나가면 보증금 받을 수 있어요?"
   - "소액보증금 최우선변제가 뭐예요?"
   - "내용증명이 뭔가요?"  ← '무엇인지' 묻는 것은 상담입니다!

2. DOCUMENT (문서 작성 요청)
   보증금 반환을 위한 "문서를 대신 써달라"는 경우
   예:
   - "내용증명 써주세요"
   - "내용증명 작성해줘"
   - "집주인한테 보낼 서류 만들어줘"
   - "보증금 반환 요구 편지 작성하고 싶어요"
   - "계약서 사진 올릴 테니 내용증명 만들어줘"

3. IRRELEVANT (범위 밖)
   보증금·경매·배당과 무관한 경우
   예:
   - 일상 대화, 날씨, 프로그래밍
   - 계약 체결·전입신고·대항력 (카테고리1)
   - 계약갱신·차임 인상 (카테고리2)
   - 전세사기 형사 문제 (카테고리4)

【판단 요령】
- "~가 뭐예요?", "~는 어떻게 하나요?" → 설명을 원함 → CONSULTATION
- "~ 써주세요", "~ 작성해줘", "~ 만들어줘" → 문서 생성을 원함 → DOCUMENT
- 애매하면 CONSULTATION으로 (문서 작성은 명확한 요청일 때만)

【사용자 질문】
{question}

【답변】
CONSULTATION, DOCUMENT, IRRELEVANT 중 하나의 단어만 출력하세요.
다른 설명은 하지 마세요."""
